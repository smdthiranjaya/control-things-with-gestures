                    ...
                    if total_fingers == 0:
                        ...
                        try:
                            ...
                            if settings.get("detect_led1", True):
                                requests.get(f'http://{esp_ip}/led1/off', timeout=0.5)
                                device_status["led1"] = "OFF"
                            if settings.get("detect_led2", True):
                                requests.get(f'http://{esp_ip}/led2/off', timeout=0.5)
                                device_status["led2"] = "OFF"
                            if settings.get("detect_motor", True):
                                requests.get(f'http://{esp_ip}/motor/off', timeout=0.5)
                        ...
                        try:
                            esp_ip = get_esp8266_ip()
                            if settings.get("detect_led1", True):
                                requests.get(f'http://{esp_ip}/led1/on', timeout=0.5)
                                device_status["led1"] = "ON"
                            if settings.get("detect_led2", True):
                                requests.get(f'http://{esp_ip}/led2/on', timeout=0.5)
                        ...
                        if settings.get("detect_led1", True):
                            ...
                            try:
                                esp_ip = get_esp8266_ip()
                                requests.get(f'http://{esp_ip}/led1_toggle', timeout=0.5)
                        ...
                        if settings.get("detect_led2", True):
                            ...
                            try:
                                esp_ip = get_esp8266_ip()
                                requests.get(f'http://{esp_ip}/led2_toggle', timeout=0.5)
                                device_status["led2"] = new_state
                            except:
                                pass
                    
                    elif total_fingers == 3:
                        # 3 fingers - Turn ON motor and buzzer, turn OFF both LEDs
                        if settings.get("detect_motor", True):
                            print("[GESTURE] 3 Fingers - Motor & Buzzer ON, LEDs OFF")
                            try:
                                esp_ip = get_esp8266_ip()
                                # Turn ON motor
                                requests.get(f'http://{esp_ip}/motor/on', timeout=0.5)
                        ...
                        # Any other finger count - turn motor OFF if it was ON
                        if settings.get("detect_motor", True) and device_status.get("motor") == "ON":
                            print("[GESTURE] Motor OFF (gesture changed)")
                            try:
                                esp_ip = get_esp8266_ip()
                                requests.get(f'http://{esp_ip}/motor/off', timeout=0.5)
                                device_status["motor"] = "OFF"
